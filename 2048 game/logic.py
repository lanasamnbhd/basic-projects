...
def findEmpty(mat):
    #finds the first empty (0) cell in grid
    for i in range(4):
        for j in range(4):
            if mat[i][j]==0:
                return i,j
...
def get_current_state(mat):
    ...
    for i in range(4):
     ...
     #if we are still left with at least one empty cell
...
def compress(mat):
    ...
    for i in range(4):
        ...
        for j in range(4):
            if(mat[i][j]!=0):
                #if cell is non empty then we will shift it's number to previous empty cell in that row
...
#means reversing the content of each row
def reverse(mat):
    new_mat=[]
    for i in range(4):
        new_mat.append([])
